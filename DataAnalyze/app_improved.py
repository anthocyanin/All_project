...
def index():
    ...
    if not username:
        return redirect(url_for('login'))
    payments = bank.get_payment_of(bank.open_database(), username)
    return render_template('index.html', payments=payments, username=username, flash_message=get_flashed_messages())
...
def pay():
    ...
    if request.method == 'POST':
        ...
        if account and dollars and dollars.isdigit() and memo:
            ...
            db.commit()
            flash('Payment sucessful')
            return redirect(url_for('index'))
        complaint = ('Dollars must be an integer' if not dollars.isdigit() else 'Please fill in all three fields')
    return render_template('pay2.html', complaint=complaint, account=account, dollars=dollars, memo=memo,
